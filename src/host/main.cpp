...
Author: David Akre
Date: 2/17/24
Description: Main source code for polling cameras (camera module3 and luxonis
cameras), computing optical flow, and storing results in shared memory
*/
...
int main(int argc, char *argv[]) {
  ...
  for (auto &cm : camera_modules) {
    threads.emplace_back([&]() { cm->Run(frame_rate); });
  }
